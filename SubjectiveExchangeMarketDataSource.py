import time
from subjective_abstract_data_source_package.SubjectiveDataSource import SubjectiveDataSource
from brainboost_data_source_logger_package.BBLogger import BBLogger


class SubjectiveExchangeMarketDataSource(SubjectiveDataSource):
    connection_type = "ExchangeAPI"
    connection_fields = ["exchange", "api_key", "api_secret", "passphrase", "base_url"]
    icon_svg = "<svg width='24' height='24' viewBox='0 0 24 24' xmlns='http://www.w3.org/2000/svg'><circle cx='12' cy='12' r='9' fill='#2d6a4f'/><path d='M7 12h10' stroke='#ffffff' stroke-width='2'/></svg>"

    def get_icon(self):
        return self.icon_svg

    def get_connection_data(self):
        return {"connection_type": self.connection_type, "fields": list(self.connection_fields)}

    def _get_param(self, key, default=None):
        return self.params.get(key, default)

    def _emit_result(self, result):
        if result is None:
            self.set_total_items(0)
            self.set_processed_items(0)
            return
        if isinstance(result, (list, tuple)):
            self.set_total_items(len(result))
            self.set_processed_items(0)
            for item in result:
                self.update(item)
                self.increment_processed_items()
            return
        self.set_total_items(1)
        self.set_processed_items(0)
        self.update(result)
        self.increment_processed_items()

    def fetch(self):
        start = time.perf_counter()
        if self.status_callback:
            self.status_callback(self.get_name(), "fetch_started")
        from com_goldenthinker_trade_exchange.ExchangeConfiguration import ExchangeConfiguration
        from com_goldenthinker_trade_model.Symbol import Symbol

        exchange = ExchangeConfiguration.get_default_exchange()
        action = self._get_param("action", "all_tickers")
        symbol_value = self._get_param("symbol")
        symbol = Symbol(symbol_value) if symbol_value else None

        if action == "all_tickers":
            data = exchange.get_all_tickers()
        elif action == "current_price":
            if symbol is None:
                raise ValueError("symbol is required for current_price")
            data = {"symbol": symbol.uppercase_format(), "price": exchange.get_current_price(symbol)}
        elif action == "symbol_info":
            if symbol is None:
                raise ValueError("symbol is required for symbol_info")
            data = exchange.get_symbol_information(symbol)
        elif action == "orders":
            if symbol is None:
                raise ValueError("symbol is required for orders")
            data = exchange.get_all_orders(symbol)
        elif action == "balance":
            if symbol is None:
                raise ValueError("symbol is required for balance")
            base = bool(self._get_param("base", False))
            data = exchange.balance(symbol, base=base)
        elif action == "portfolio":
            data = exchange.portfolio()
        elif action == "klines":
            if symbol is None:
                raise ValueError("symbol is required for klines")
            date_a = self._get_param("date_a")
            date_b = self._get_param("date_b")
            if not date_a or not date_b:
                raise ValueError("date_a and date_b are required for klines")
            data = exchange.get_klines(symbol, date_a, date_b)
        else:
            raise ValueError(f"Unsupported action: {action}")

        self._emit_result({"action": action, "data": data})
        duration = time.perf_counter() - start
        self.set_total_processing_time(duration)
        self.set_fetch_completed(True)
        if self.progress_callback:
            self.progress_callback(self.get_name(), self.get_total_to_process(), self.get_total_processed(), self.estimated_remaining_time())
        if self.status_callback:
            self.status_callback(self.get_name(), "fetch_completed")
        BBLogger.log(f"Fetch completed for {self.get_name()}")
